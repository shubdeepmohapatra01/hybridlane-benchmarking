    ...
    *machine* has a GPU (via ``CUDA_VISIBLE_DEVICES``, else ``nvidia-smi``).
    ...
    if strict and info["requested"] == "gpu" and info["platform"] != "gpu":
        raise RuntimeError(
            ...
            "Two usual causes:\n"
            "  1. CUDA-enabled JAX is not installed. The default install is "
            "CPU-only; the GPU deps are kept out of pyproject.toml so this fork "
            "stays mergeable with upstream. Install them with\n"
            "       python -m pip install -r hpc/requirements-hpc.txt\n"
            "     and confirm with `python -c \"import jax; print(jax.devices())\"`.\n"
            "     The jax<0.11 pin there is load-bearing -- see hpc/README.md.\n"
