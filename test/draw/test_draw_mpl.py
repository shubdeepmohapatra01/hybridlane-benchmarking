...
from hybridlane.drawer.mpldrawer import _icon_face_color  # noqa: E402
...
def test_icon_colors():
    ...
    for input in (circuit1, qp.QNode(circuit1, dev)):
        ...
        assert icon_patches[0].get_facecolor()[:3] == _icon_face_color(default_qubit_color, 0.3)

        # Check qumode wire 1
        assert isinstance(icon_patches[1], patches.Rectangle)
        assert icon_patches[1].get_facecolor()[:3] == _icon_face_color(default_qumode_color, 0.3)

        # Check the customized qumodes
        for i in range(2, 7):
            assert isinstance(icon_patches[i], patches.Rectangle)
            assert icon_patches[i].get_facecolor()[:3] == _icon_face_color(icon_colors[i], 0.3)

        plt.close("all")


@pytest.mark.unit
@pytest.mark.parametrize("f,wire_order", [(circuit2, (1, 0, 2)), (circuit3, ("m", 0, 2))])
def test_overlapping_qcond_warns(f, wire_order):
    ...
    for input in (f, qp.QNode(f, dev)):
        ...
        with pytest.warns(UserWarning):
            hl.draw_mpl(input, wire_order=wire_order)()
