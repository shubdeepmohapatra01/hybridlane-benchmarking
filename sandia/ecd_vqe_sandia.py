...
_PROBLEMS = {
    ...
    # Added for the depth/iteration study, which needs a point between 8 and 12
    # -- the jump from a 5% random-start success rate at 8 variables to ~4% at
    # 12 is too coarse to locate where the ansatz starts failing. Generated by
    # `knapsack_scaling.design_instance(10)` on the same seed 0 as the rest of
    # the series, so it shares their design constraints exactly: unique optimum,
    # gap 8.0, binding capacity, and target=(1, 3, 1) as at 8 and 12. Its
    # simulation cost (64 x 128) equals theirs, so it is not a slower point.
    "knapsack_n10": dict(
        n_items=6, values=[8, 9, 2, 8, 3, 9], weights=[5, 5, 7, 7, 7, 4],
        max_weight=15, l_val=10,
        n_bits_m0=5, n_bits_m1=4,
        h_opt=-26.0, target=(1, 3, 1), n_depth=7,
        max_fock=(64, 128),
    ),
    # gap=8.0  packed=14/15  target=(1, 3, 1)  n_vars=10
    "knapsack_n12": dict(
